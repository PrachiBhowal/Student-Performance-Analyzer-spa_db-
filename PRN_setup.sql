...
CREATE TABLE faculty_subjects (
    ...
    CONSTRAINT fk_faculty_subjects_user
...
CREATE TABLE student_subject_marks (
    ...
    CONSTRAINT fk_ssm_student
        FOREIGN KEY (student_id) REFERENCES students(id)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT fk_ssm_subject
        FOREIGN KEY (subject_id) REFERENCES subjects(subject_id)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    CONSTRAINT fk_ssm_faculty
        FOREIGN KEY (faculty_user_id) REFERENCES users(id)
        ON UPDATE CASCADE
        ON DELETE CASCADE
) ENGINE=InnoDB;
...
('Artificial Intelligence', 'AIML', 4),
('Software Engineering', 'CSE', 3),
('Cloud Computing', 'CST', 3);
